...
The Autodev Goal Contract is a local long-horizon continuity layer. It does not
replace manifests, plans, receipts, review handoffs, git history, or other
durable repo state. If Codex exposes a documented noninteractive `/goal`
transport in the future, `src/codex_exec.rs` is the intended adapter point.
