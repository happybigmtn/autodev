...
- Generation authoring and independent review defaults currently use `gpt-5.6-sol` with
  `xhigh`.
...
- Loop, parallel, QA, QA-only, health, review, ship, steward, audit, bug, and
  Nemesis Codex defaults currently use `gpt-5.6-sol` with `high` unless a phase has
