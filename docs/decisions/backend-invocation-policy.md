...
| --- | --- | --- | --- | --- | --- | --- |
| `src/codex_exec.rs` shared Codex runner | `<codex_bin> exec --json --dangerously-bypass-approvals-and-sandbox --skip-git-repo-check --cd <repo> -m <model> -c model_reasoning_effort="<effort>"`; `run_codex_exec_max_context` also adds `-c model_context_window=1000000` | Caller supplies `model`, `reasoning_effort`, and `codex_bin` | Uses `quota_exec::run_with_quota(Provider::Codex, ...)` when Codex quota accounts are configured; otherwise spawns the configured Codex binary directly | Prompt is written by callers. Stdout is streamed through `codex_stream` and optionally mirrored to a caller-provided stdout log. Stderr is captured and appended to the caller-provided stderr log. Optional worker pid file is written and cleared. | No internal timeout. Caller owns retry or timeout. | Always bypasses Codex approvals and sandbox and skips git repo check. |
...
| Surface | Binary and argv shape | Model and effort source | Quota behavior | Output and logs | Timeout or futility | Dangerous posture |
| --- | --- | --- | --- | --- | --- | --- |
| `src/generation.rs` author routing through `run_logged_author_phase` | Routes Claude-like models to the direct Claude path below; all other model names route to shared Codex max-context execution | `CorpusArgs` and `GenerationArgs` currently default authoring to `gpt-5.5` with `xhigh`; explicit operator flags are passed through | Codex authoring uses shared Codex quota behavior. Claude authoring does not use `src/claude_exec.rs`, so it does not use quota routing | Writes `.auto/logs/<phase>-<timestamp>-prompt.md`. Codex authoring also writes stdout and stderr logs beside the prompt. Claude authoring writes a response file only when stdout is non-empty. | Codex path has no timeout. Direct Claude path has `--max-turns` but no stream futility detector. | Codex path uses sandbox bypass through the shared wrapper. Claude path skips permissions. |
...
| `src/generation.rs` Codex review | Shared Codex max-context execution | `codex_review_model` and `codex_review_effort`; current defaults are `gpt-5.5` and `xhigh` | Shared Codex quota behavior | Writes prompt log, stderr log, and requires the requested report path to be created and non-empty | No internal timeout | Codex sandbox bypass through shared wrapper. |

Generation routing must continue to send non-Claude names such as `gpt-5.5`,
`gpt-5.4`, and `o3` through Codex rather than direct Claude. This is a current
behavior constraint, not a new abstraction decision.

## Commands Using Shared Wrappers

| Surface | Binary and argv shape | Model and effort source | Quota behavior | Output and logs | Timeout or futility | Dangerous posture |
| --- | --- | --- | --- | --- | --- | --- |
| `src/loop_command.rs` | `--claude` selects shared Claude; otherwise shared Codex | `LoopArgs.model`, `LoopArgs.reasoning_effort`, optional Claude `max_turns`, and `codex_bin` | Shared wrapper behavior for the selected provider | Writes `.auto/logs/loop-<timestamp>-prompt.md`; stderr log is `.auto/loop/codex.stderr.log` by default even when Claude is selected | Claude can return futility marker `137`; loop tracks consecutive failures and distinguishes futility. No Codex timeout. | Inherits selected wrapper dangerous flags. |
| `src/parallel_command.rs` serial loop worker | `--claude` selects shared Claude with lane env; otherwise shared Codex with lane env | Host or lane config supplies `model`, `reasoning_effort`, optional Claude `max_turns`, and `codex_bin` | Shared wrapper behavior for selected provider | Writes loop prompt under `.auto/logs`; stderr goes to the run-root log path. Extra lane env is passed through. | Claude futility marker is detected and classified. No Codex timeout. | Inherits selected wrapper dangerous flags. |
| `src/parallel_command.rs` lane worker | `--claude` selects shared Claude with lane env and worker pid path; otherwise shared Codex with lane env and worker pid path | Lane config supplies `model`, `reasoning_effort`, optional Claude `max_turns`, and `codex_bin` | Shared wrapper behavior for selected provider | Writes per-attempt prompt, stdout log, stderr log, and worker pid under the lane run root | Claude futility marker is detected by the host. No Codex timeout. | Inherits selected wrapper dangerous flags. |
| `src/review_command.rs` | `--claude` selects shared Claude with review futility threshold; otherwise shared Codex max-context execution | `ReviewArgs.model`, `ReviewArgs.reasoning_effort`, optional Claude `max_turns`, and `codex_bin` | Shared wrapper behavior for selected provider | Writes `.auto/logs/review-<timestamp>-prompt.md`; stderr log is `.auto/review/codex.stderr.log` by default | Claude uses the review futility threshold. Codex has no timeout. Review also has stale-batch handling outside the backend runner. | Inherits selected wrapper dangerous flags. |
| `src/qa_command.rs` | Shared Codex execution | `QaArgs.model`, `QaArgs.reasoning_effort`, and `codex_bin` | Shared Codex quota behavior | Writes prompt under `.auto/logs`; stderr log is `.auto/qa/codex.stderr.log` by default | No internal timeout | Codex sandbox bypass through shared wrapper. |
| `src/qa_only_command.rs` | Shared Codex execution | `QaOnlyArgs.model`, `QaOnlyArgs.reasoning_effort`, and `codex_bin` | Shared Codex quota behavior | Writes prompt under `.auto/logs`; stderr log is `.auto/qa-only/codex.stderr.log` by default | No internal timeout | Codex sandbox bypass through shared wrapper. |
| `src/health_command.rs` | Shared Codex execution | `HealthArgs.model`, `HealthArgs.reasoning_effort`, and `codex_bin` | Shared Codex quota behavior | Writes prompt under `.auto/logs`; stderr log is `.auto/health/codex.stderr.log` by default | No internal timeout | Codex sandbox bypass through shared wrapper. |
| `src/ship_command.rs` | Shared Codex execution | `ShipArgs.model`, `ShipArgs.reasoning_effort`, and `codex_bin` | Shared Codex quota behavior | Writes prompt under `.auto/logs`; stderr log is `.auto/ship/codex.stderr.log` by default | No internal timeout | Codex sandbox bypass through shared wrapper. |
| `src/steward_command.rs` | Shared Codex execution for steward and finalizer phases | `StewardArgs.model` and `reasoning_effort`; finalizer uses `finalizer_model` and `finalizer_effort`; both use `codex_bin` | Shared Codex quota behavior | Writes prompts under `.auto/logs`; stderr logs are under the steward output directory | No internal timeout | Codex sandbox bypass through shared wrapper. |
| `src/super_command.rs` | Shared Codex max-context execution for super-only review gates | `SuperArgs` phase model and effort values plus `codex_bin` | Shared Codex quota behavior | Writes prompt and stderr logs under the super run root | No internal timeout | Codex sandbox bypass through shared wrapper. |
...
- Generation authoring and Codex review defaults currently use `gpt-5.5` with
  `xhigh`.
