...
Overarching objective (the default north-star when no idea seed overrides it): drive this repository toward a PRODUCTION-GRADE, SCALABLE implementation of its product running end-to-end on a public testnet. The corpus must chart the shortest credible path to that outcome and rank every focus area by how directly it advances a real, user- or operator-exercisable product/runtime/protocol capability on that path. Guard scripts, receipts, audits, evidence rollups, CI plumbing, and process bookkeeping are NEVER a top focus area unless they directly unblock a named product capability on that path.
