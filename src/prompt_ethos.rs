...
- Treat `auto design` as a first-class production lever, not a mockup side quest: it should run before `auto gen` or `auto parallel` when UI/TUI/frontend quality, presentation source-of-truth, or shared component contracts are the highest blocker. Recommend `auto design --apply` for doctrine/queue repair and `auto design --resolve` when design findings need implementation lanes.
