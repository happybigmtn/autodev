...
- Treat `auto design` as a first-class production lever, not a mockup side quest: run it before `auto gen` or `auto parallel` when UI/TUI/frontend quality, presentation source-of-truth, or shared component contracts are the highest blocker. Use `auto design --apply` for doctrine/queue repair and `auto design --resolve` when design findings need implementation lanes.
...
- For TUI work, require terminal-native proof: stable breakpoint plates, buffer/cell assertions, tokenized typography/color, input-state ergonomics, and post-render animation proof when motion is used. Do not accept one-off screenshots without reusable Ratatui-style layout/component contracts."#;
...
mod tests {
    ...
    fn ethos_is_prepended_once() {
        ...
        assert!(super::PRODUCTION_ORCHESTRATION_DISCIPLINE.contains("terminal-native proof"));
