...
fn finalize_task(lines: &[String]) -> Option<PlanTask> {
    ...
    Some(PlanTask {
        ...
        lane_kind: parse_lane_kind(&markdown),
        markdown,
        body,
    })
}
...
fn parse_lane_kind(markdown: &str) -> Option<LaneKind> {
    let value = task_field_body_until_any(markdown, "Lane kind:", TASK_FIELD_BOUNDARIES)?;
    let first = value
        .lines()
        .map(strip_list_bullet)
        .map(str::trim)
        .find(|line| !line.is_empty())?;
    LaneKind::parse(first)
}
...
pub(crate) fn task_field_line_remainder<'a>(line: &'a str, field: &str) -> Option<&'a str> {
    let trimmed = line.trim_start();
...
- [ ] `AD-001` Pending parser inventory
  ...
  Verification: `cargo test task_parser::parse::tests::parses_all_plan_statuses_and_fields`
...
- [~] `AD-002` Historical parser gap
  ...
  Verification:
  - `cargo test task_parser::parse::tests::dependencies_none_and_multiline_notes_are_stable`
...
"#;
        ...
        assert_eq!(
            ...
            Some("`cargo test task_parser::parse::tests::parses_all_plan_statuses_and_fields`")
        );
        assert_eq!(
            tasks[0].completion_artifacts,
