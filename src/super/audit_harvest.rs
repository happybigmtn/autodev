// Audit harvest stage: pull `analysis.json` rows from an `auto audit
// --everything` run, classify them via harvest_cluster, route non-single-row
// clusters into typed queues, and ask codex to append concrete plan rows for
// the residue. Spliced into `super_command.rs` via `include!`.
