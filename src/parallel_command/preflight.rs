...
pub(crate) struct ParallelPreflightReport {
    pub(crate) checks: Vec<ParallelPreflightCheck>,
}
...
impl ParallelPreflightReport {
    ...
    pub(crate) fn prompt_clause(&self) -> String {
        self.checks
            .iter()
            .map(|check| {
                format!(
...
pub(crate) struct ParallelPreflightCheck {
...
pub(crate) fn run_parallel_preflight(
    repo_root: &Path,
...
pub(crate) fn contains_any_term(haystack: &str, needles: &[&str]) -> bool {
    needles.iter().any(|needle| contains_term(haystack, needle))
}

pub(crate) fn is_term_char(ch: char) -> bool {
    ch.is_ascii_alphanumeric() || matches!(ch, '_' | '-')
}

#[derive(Clone, Copy, Debug, Eq, PartialEq)]
pub(crate) struct ParallelPreflightNeeds {
    pub(crate) browser: bool,
    pub(crate) docker: bool,
    pub(crate) regtest: bool,
}

pub(crate) fn classify_parallel_preflight_needs(
    task_text: &str,
    repo_root: &Path,
...
pub(crate) fn command_stdout<const N: usize>(cwd: &Path, args: [&str; N]) -> Result<String> {
...
pub(crate) fn default_agent_browser_socket() -> PathBuf {
...
pub(crate) fn try_warm_agent_browser_daemon(repo_root: &Path, socket: &Path) -> bool {
